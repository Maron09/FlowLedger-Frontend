...
export default function BudgetsPage() {
  ...
  const openEdit = (bs: BudgetStatus) => {
    setEditBudget(bs)
    setForm({
      categoryId: bs.budget.category.id,
      amount: String(Number(bs.budget.amount)),
    })
    setError('')
    setShowForm(true)
  }
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <select
                  ...
                  required
                  disabled={!!editBudget}
                  className="w-full bg-[#0f1117] border border-white/10 rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:border-emerald-500/50 transition-all disabled:opacity-50"
                >
                  <option value="" style={{ backgroundColor: '#0f1117' }}>Select category</option>
                  {availableCategories.map((cat) => (
                    <option key={cat.id} value={cat.id} style={{ backgroundColor: '#0f1117' }}>
                      {cat.name}
                    </option>
                  ))}
                </select>
              ...
              <div className="flex gap-3 pt-1">
                ...
                >
                  {saving ? 'Saving...' : editBudget ? 'Update' : 'Save Budget'}
                </button>
              </div>
            </form>
          </div>
